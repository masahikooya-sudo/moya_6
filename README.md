...
アップロードすると、以下の15種類の個人情報がまだ残っていないかを確認できます。

人物名 / フリガナ / 住所・地名 / 企業名・組織名 / 日付 / 時刻 / 金額 / 数量 /
電話番号 / メールアドレス / マイナンバー / 郵便番号 / クレジットカード番号 /
銀行口座番号 / パスポート番号

チェックは3つの方法を組み合わせて行います。

1. **正規表現によるパターンマッチング**(メールアドレス、電話番号、郵便番号、
   マイナンバー、クレジットカード番号(Luhnアルゴリズムで検証)、銀行口座番号、
   パスポート番号、日付・時刻・金額・数量の代表的な表記など)。
2. **xlsxの列見出しによる検出**(「氏名」「フリガナ」「住所」「電話番号」のような
   典型的な名簿・台帳形式のファイルの場合、1行目の列見出しからその列全体が
   何のデータかを機械的に判定し、該当列の全セルをその種類として検出します。
   正規表現では検出できない人物名・フリガナ・住所・企業名も、Ollamaが使えない
   状況で一定精度で検出できます)。
3. **AI(Gemma)による確認**(ファイルからテキストを抽出し、Ollama経由でGemmaに
   人物名・フリガナ・住所・企業名なども含めた15種類すべての確認を依頼)。

検出結果は種類・検出内容・検出方法(パターン検出/列見出し検出/AI検出)・場所(xlsxは
シート名とセル番地、pdfはページ番号)を一覧表示します。Ollamaが起動していない場合は
正規表現と列見出しによる結果のみを表示し、その旨を警告として表示します。

> **注意:** このチェックは補助的なものであり、検出漏れ(偽陰性)が発生する可能性が
> あります。列見出しによる検出は列見出しが「氏名」「住所」のような典型的な文言である
> 場合にのみ機能し、見出しのない表や自由記述のテキスト(pdf等)では機能しません。
> それらの場合は人物名・フリガナ・住所・企業名などの検出はAIの判断精度に依存するため、
> チェック結果にかかわらず、必ず目視でも内容を確認してください。アップロードできる
> ファイルは `.xlsx` と `.pdf` のみで、ファイルサイズの上限は50MBです。
