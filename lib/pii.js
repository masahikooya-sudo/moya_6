...
export const PII_CATEGORIES = [
  { key: 'person_name', label: '人物名', method: 'pattern+llm' },
  { key: 'furigana', label: 'フリガナ', method: 'llm' },
  { key: 'address', label: '住所・地名', method: 'llm' },
  { key: 'organization', label: '企業名・組織名', method: 'pattern+llm' },
...
// CPUのみの環境では1回のAI呼び出しが重いほど応答が遅くなる(タイムアウトしやすくなる)ため、
// チャンクサイズ・チャンク数の上限は環境変数で調整できるようにしている。
const CHUNK_CHAR_LIMIT = Number(process.env.PII_CHUNK_CHAR_LIMIT) || 3000;
export const MAX_CHUNKS = Number(process.env.PII_MAX_CHUNKS) || 40;
...
// 罫線で区切られた表(TableArray: 行×列の文字列の2次元配列)の1行目を見出しとみなし、
// xlsxの列見出し検出と同じ要領で、各列のカテゴリを判定して残りの行のセルを検出する。
function extractColumnFindingsFromTable(table) {
  if (!Array.isArray(table) || table.length < 2) return [];
  const header = table[0];
  const columnCategories = header.map((cell) => matchColumnCategory(String(cell || '')));
  if (!columnCategories.some((c) => c)) return [];

  const findings = [];
  for (const row of table.slice(1)) {
    row.forEach((cell, colIndex) => {
      const category = columnCategories[colIndex];
      const text = String(cell || '').trim();
      if (category && text) findings.push({ category, text });
    });
  }
  return findings;
}
...
export async function extractPdf(buffer) {
  ...
  try {
    ...
    // 罫線付きの表(名簿・申込書等でよくある形式)があれば、xlsxと同じ列見出し検出を
    // 適用する。テキストに何らかの構造があっても、罫線が引かれていない表(空白や
    // インデントだけで整列された表)はここでは検出できない。
    const columnFindings = [];
    try {
      const tableResult = await parser.getTable();
      for (const page of tableResult.pages || []) {
        page.tables.forEach((table, tableIndex) => {
          const location = `ページ ${page.num} の表${tableIndex + 1}`;
          for (const f of extractColumnFindingsFromTable(table)) {
            columnFindings.push({ ...f, location });
          }
        });
      }
    } catch {
      // 表構造の検出に失敗しても、通常のテキスト抽出結果は引き続き利用できるため、
      // ここでは無視して処理を続ける。
    }
...
// 法人格を表す語を含む文字列は企業名・組織名である可能性が高いため、正規表現で
// 検出できる。日本語の法人格は会社名の前後どちらにも付くため、前置・後置の
// 両方のパターンを用意する。
const ORG_SUFFIX_ALT =
  '株式会社|有限会社|合同会社|合資会社|合名会社|一般社団法人|公益社団法人|一般財団法人|' +
  '公益財団法人|医療法人|社会福祉法人|学校法人|特定非営利活動法人|NPO法人';
const ORG_NAME_CHARS = '[一-龥ァ-ヶーa-zA-Z0-9・]';
...
const SIMPLE_PATTERNS = [
  ...
  { category: 'organization', regex: new RegExp(`(?:${ORG_SUFFIX_ALT})${ORG_NAME_CHARS}{1,20}`, 'g') },
  { category: 'organization', regex: new RegExp(`${ORG_NAME_CHARS}{1,20}(?:${ORG_SUFFIX_ALT})`, 'g') },
  { category: 'organization', regex: /\b(?:[A-Z][A-Za-z0-9&]*\.?\s+){1,5}(?:Inc\.?|LLC|Ltd\.?|Corp\.?|Co\.,?\s?Ltd\.?)\b/g },
...
// 「様」「殿」「氏」などの敬称の直前にある、漢字・カタカナのみからなる2文字以上の
// 連続部分を人物名の候補として検出する。ひらがなを対象文字から除外しているため、
// 「お客様」「皆様」のような助詞や仮名を含む一般的な言い回しは、敬称の直前が
// 1文字以下になり自然に除外される。「担当者様」「関係者各位」のような、ひらがなを
// 挟まずに敬称へ繋がる組織・役割名は誤検出しやすいため、代表的なものを除外する。
const HONORIFIC_RE = /([一-龥ァ-ヶー]{2,10})(様|殿|氏)/g;
const HONORIFIC_BLOCKLIST = new Set([
  '各位', '担当者', 'ご担当者', '関係者', '利用者', 'ご利用者', '参加者', 'ご参加者',
  '保護者', '従事者', '責任者', '主催者', '出席者', '対象者', '代表者', '申込者', '希望者',
]);

function scanHonorificNames(text) {
  const findings = [];
  for (const match of text.matchAll(HONORIFIC_RE)) {
    const namePart = match[1];
    if (HONORIFIC_BLOCKLIST.has(namePart)) continue;
    findings.push({ category: 'person_name', text: match[0] });
  }
  return findings;
}

// 敬称が付いていない氏名を検出するため、代表的な日本人の名字(2文字以上のもの)を
// 手がかりにする。「森」「林」「田」のような1文字の名字は一般名詞と紛らわしく
// 誤検出が多くなるため対象外にしている。実在する名字を網羅するものではなく、
// 頻度の高いものに限定した補助的な仕組み。
const COMMON_SURNAMES = [
  '佐藤', '鈴木', '高橋', '田中', '伊藤', '渡辺', '山本', '中村', '小林', '加藤',
  '吉田', '山田', '佐々木', '山口', '松本', '井上', '木村', '斎藤', '斉藤', '清水',
  '山崎', '森田', '阿部', '池田', '橋本', '山下', '石川', '中島', '前田', '藤田',
  '後藤', '岡田', '長谷川', '村上', '近藤', '石井', '坂本', '遠藤', '青木', '藤井',
  '西村', '福田', '太田', '三浦', '岡本', '松田', '中川', '中野', '原田', '小川',
  '竹内', '金子', '和田', '中山', '石田', '上田', '柴田', '酒井', '工藤', '横山',
  '宮崎', '宮本', '内田', '高木', '安藤', '島田', '谷口', '大野', '高田', '丸山',
  '今井', '河野', '藤原', '村田', '上野', '杉山', '増田', '小野', '田村', '大塚',
  '高山', '平野', '小島', '田口', '浅野', '千葉', '武田', '新井', '岩崎', '桜井',
  '松井', '木下', '野口', '松尾', '菅原', '久保', '佐野', '岩本', '望月', '大西',
  '服部', '荒木', '松岡', '三宅', '秋山', '中西', '小山', '石橋', '川崎', '熊谷',
  '大石', '三上', '平田', '本田', '竹田', '川口', '森本', '吉川', '大久保', '白石',
  '安田', '星野', '片山', '篠原', '永井', '山中', '小松', '荒井', '川上', '川村',
  '中田', '山内', '関口', '宮田', '杉本', '土屋', '小泉', '菊地', '菊池', '須藤',
  '梅田', '広瀬', '西田', '古川',
];
const SURNAME_ALT = COMMON_SURNAMES.join('|');
// 「高橋川」「山口県」のように、名字が地名・施設名の一部になっているケースを
// 誤って人名として検出しないよう、名字の直後がこれらの文字で始まる場合は
// 下の名前の候補として取り込まない。「様」「殿」「氏」も、敬称検出(scanHonorificNames)
// と重複しないようここで除外する。
const GEO_SUFFIX_CHARS = '川山県市区町村駅線通店場銀病学神社公園丘台原谷浜港島郡様殿氏';
// 名字の前後が漢字で連続していない(=単語の区切りになっている)ことを条件にし、
// より長い熟語や地名の一部分だけを切り出してしまうのを防ぐ。名字の直後に続く
// 1〜3文字の漢字は名前(下の名前)の候補として一緒に検出する。
const SURNAME_NAME_RE = new RegExp(
  `(?<![一-龥])(?:${SURNAME_ALT})(?:(?:(?![${GEO_SUFFIX_CHARS}])[一-龥]){1,3})?(?![一-龥])`,
  'g'
);

function scanSurnameNames(text) {
  const findings = [];
  for (const match of text.matchAll(SURNAME_NAME_RE)) {
    findings.push({ category: 'person_name', text: match[0] });
  }
  return findings;
}
...
export function scanTextWithPatterns(text) {
  return [
    ...scanSimplePatterns(text),
    ...scanNumericSequences(text),
    ...scanHonorificNames(text),
    ...scanSurnameNames(text),
  ];
}
